...
- LLM quantization
  > Please download the quantized weights [modelscope/Eco-Tech](https://modelscope.cn/models/Eco-Tech) first.

  _Adding the real folder structure here would improve readability.here is a simple example, please update it blow with the output from the `tree` command_
  ```bash
   Qwen3-32B/      # floating-point model downloaded from official HF repo
   calibration_data/    # some description
      |---some file(such as asboolq.jsonl)
